...
@st.cache_data
def load_data():
    df=pd.read_csv("C:/Users/DELL/Documents/Ferry Capacity Utilization & Operational Efficiency Analytics System/Toronto Island Ferry Tickets.csv")
...
if page=="Dashboard":
    ...
    if resolution=='Daily':
        ...
        fig=px.bar(hourly,x='Hour',y='Total',)
    ...
    with col1:
        ...
        fig=px.imshow(heatmap_data,title='Acitivity Heatmap',color_continuous_scale='Reds')
        st.plotly_chart(fig,use_container_width=True)
    
    with col2:
        comparision=df_filtered.groupby('IsWeekend')['Total'].mean()
        fig=px.bar(x=['Weekday','Weekend'],y=comparision.values,title='Weekday vs Weekend')
        st.plotly_chart(fig,use_container_width=True)

    if congestion>0.2:
        st.warning("High congestion detected! Consider adding more ferries during peak hours!!!")
    if idle > 0.3:
        st.info("High idle periods detected! Consider reducding service during off-peak hours!!!")
    ...
    with col1:
        ...
        st.plotly_chart(fig,use_container_width=True)
    with col2:
        fig=px.bar(seasonal,x='Season',y='max',title='Peak Acitivity by Season')
        st.plotly_chart(fig,use_container_width=True)
    ...
    fig.update_layout(title='Hourly Acitivty Pattern',xaxis_title='Hour',yaxis_title='Average Tickets')
    st.plotly_chart(fig,use_container_width=True)
    ...
    savings={
        "Category":['Fuel Optimization','Labor Efficiency','Maintainance Reduction'],
